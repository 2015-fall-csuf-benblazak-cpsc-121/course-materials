...
\begin{itemize}
  \item Create and initialize (in the same statement) an object named
    ...
    values \mintinline{cpp}{4}, \mintinline{cpp}{3}, and \mintinline{cpp}{2}
    (in that order).
  \item Use the method on \mintinline{cpp}{vector<int>} that adds an element to
    ...
    add \mintinline{cpp}{1} to the end of \mintinline{cpp}{v}.
  \item Use the \mintinline{cpp}{[]} operator to change the element at index
    \mintinline{cpp}{1} to \mintinline{cpp}{7}.
...
\textAnswer{\showCodeTop{standard-1}{all}}
