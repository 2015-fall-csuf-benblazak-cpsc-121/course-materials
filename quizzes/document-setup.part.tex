...
\def \docCodeDir {./code/\directlua{strip_extensions('\jobname')}/}
\def \docImageDir {./images/\directlua{strip_extensions('\jobname')}/}
...
\def \evaluateCodeTop #1{
  \par
  \inputminted{cpp}{\docCodeDir/#1.cpp}
  \textQuestion{
    \inputminted[label={\normalsize{}Output},fontsize=\Large]{text}
      {\docCodeDir/#1.gen.output.gen.blank}
  }
  \textAnswer{
    \inputminted[label={\normalsize{}Output},fontsize=\Large]{text}
      {\docCodeDir/#1.gen.output}
  }
  \par
}
\def \evaluateCodeLeft #1{
  \par
  ...
  \begin{minipage}[t]{0.5\linewidth} \vspace{0ex}
    \textQuestion{
      \inputminted[label=Output]{text}{\docCodeDir/#1.cpp.gen.blank}
    }
    \textAnswer{
      \inputminted[label=Output]{text}{\docCodeDir/#1.gen.output}
    }
  \end{minipage}
  \par
}
...
\def \showCodeTop #1#2{{
  \if\relax\noexpand#2\relax
    \def\suffix{.gen.section}
  \else
    \def\suffix{.gen.section-#2}
  \fi
  \par
  \inputminted{cpp}{\docCodeDir/#1.cpp\suffix}
  \vspace{0.5\parskip}
  \inputminted[label=Output]{text}{\docCodeDir/#1.gen.output}
  \par
}}
