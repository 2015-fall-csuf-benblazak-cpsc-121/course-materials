...
\def \myCodeDir {./code/\directlua{doc_name('\jobname')}/}
\def \myImageDir {./images/\directlua{doc_name('\jobname')}/}
...
\def \code #1{
  \inputminted[frame=single]{cpp}{\myCodeDir/#1}
}
\def \output #1{
  \inputminted[frame=single,label={Output}]{cpp}{\myCodeDir/#1}
}
\def \outputLarge #1{
  \inputminted[frame=single,label={\normalsize{}Output},fontsize=\Large]{cpp}{\myCodeDir/#1}
}
\def \codeTop #1{
  \par
  \code{#1.cpp}
  \textQuestion{
    \write18{sed 's/.*/ /' '#1.gen.txt' > '#1.temp.txt'}
    \outputLarge{#1.temp.txt}
    \write18{rm '#1.temp.txt'}
  }
  \textAnswer{
    \outputLarge{#1.gen.txt}
  }
  \par
}
\def \codeLeft #1{
  \par
  \begin{minipage}[t]{0.5\textwidth} \vspace{0ex}
    \code{#1.cpp}
  \end{minipage}
  \begin{minipage}[t]{0.5\textwidth} \vspace{0ex}
    \textQuestion{
      \write18{sed 's/.*/ /' '#1.cpp' > '#1.temp.txt'}
      \output{#1.temp.txt}
      \write18{rm '#1.temp.txt'}
    }
    \textAnswer{
